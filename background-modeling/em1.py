...
def loadData(mu0, sigma0, mu1, sigma1, alpha0, alpha1):
    ...
    # 满足alpha的作用
    data0 = np.random.normal(mu0, sigma0, int(length * alpha0)) # 300 samples
    # 第二个高斯分布的数据
    data1 = np.random.normal(mu1, sigma1, int(length * alpha1)) # 700 samples
...
def E_step(dataSetArr, alpha0, mu0, sigmod0, alpha1, mu1, sigmod1):
    ...
    # 计算y0的响应度
    # 先计算模型0的响应度的分子
    temp0 = calcGauss(dataSetArr, mu0, sigmod0)
    gamma0 = alpha0 * temp0  # 在模型0下，1000个样本的发生概率
                             # 模型0被采用的概率为 alpha0
    # 模型1响应度的分子
    temp1 = calcGauss(dataSetArr, mu1, sigmod1)
    gamma1 = alpha1 * temp1  # 在模型1下，1000个样本的发生概率
                             # 模型1被采用的概率为 alpha1
    ...
    # gamma0 = ( alpha0 * temp0 ) / ( alpha0 * temp0 + alpha1 * temp1 )
    # gamma1 = ( alpha1 * temp1 ) / ( alpha0 * temp0 + alpha1 * temp1 )
...
def EM_Train(dataSetList, iter=5000):
    '''
    根据EM算法进行参数估计
...
if __name__ == '__main__':
    ...
    print('alpha0:%.2f, mu0:%.1f, sigmod0:%.1f, alpha1:%.2f, mu1:%.1f, sigmod1:%.1f' % (
